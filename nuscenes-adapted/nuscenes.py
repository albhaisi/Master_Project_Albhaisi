...
class NuScenesExplorer:
    ...
    def list_scenes(self) -> None:
        ...
        for start_time, record in sorted(recs):
            # matlab script uses posixtime but unix is expected
            start_time = self.nusc.get('sample', record['first_sample_token'])['timestamp'] / 1#000000
            length_time = self.nusc.get('sample', record['last_sample_token'])['timestamp'] / 1 - start_time
    ...
    def render_sample_data(self,
        ...
        elif sensor_modality == 'MONO_CAM':
            # Load boxes and image.
            # data_path, boxes, camera_intrinsic = self.nusc.get_sample_data(sample_data_token,
            #                                                                box_vis_level=box_vis_level)
            print(self.nusc.dataroot)
